function PlotIrisData (w, x, y)

% This is a modified version of PlotData.m. I wanted to see all 3 groupings, and PlotData only dealt with 2 possibilities.
...
clf;
hold on;
plot(x(find(y<0),1),x(find(y<0),2),'+r', x(find(y==0),1),x(find(y==0),2),'+g', x(find(y>0),1),x(find(y>0),2),'+b');
